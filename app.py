...
with col_left:
    ...
    with tab2:
        ...
        if not weekly_df_filtered.empty:
            ...
            st.dataframe(w_leaderboard, use_container_width=True, hide_index=True)
        else:
    ...
    with tab3:
        ...
        if not filtered_df.empty:
            ...
            if not shame_df.empty:
                st.dataframe(shame_df, use_container_width=True, hide_index=True)
            else:
...
with tab_streaks:
    ...
    if not filtered_df.empty:
        ...
        if not user_dates.empty:
            ...
            st.dataframe(top_streaks, use_container_width=True, hide_index=True)
        else:
